...
const Login = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await authAxios.post(`admin-api/v1/email/verify-email/${smsId}/${OTP}`);
      const {
        data: {
          data: { access_token },
        },
      } = await authAxios.post("admin-api/v1/auth/login", {
        email: email,
        password: password,
      });
  ...
  const updateStep = async () => {
    try {
      ...
      } = await authAxios.post("admin-api/v1/email/send-otp", {
        email: email,
      });
      setSmsId(sms_id);
      setStep(2);
